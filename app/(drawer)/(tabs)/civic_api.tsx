...
const CivicAPI = () => {
    ...
    const getInfoForLocation = useCallback(async () => {
        try {
            ...
            const requestBody = {
                address: encodeURIComponent("2101 E NASA Pkwy, Houston, TX")
            };

            console.log(`https://www.googleapis.com/civicinfo/v2/voterinfo?key=${apiKey}&address=${requestBody.address}&electionId=8123`);

            const response = await fetch(`https://www.googleapis.com/civicinfo/v2/voterinfo?key=${apiKey}&address=${requestBody.address}&electionId=8123`, {
                headers: {
                ...
                <YStack flex={1} alignItems="center" justifyContent="center">
                    ...
                    {civicData && (
                        <View margin={5}>
                            ...
                            <Text>Location:
                                {civicData.state[0].electionAdministrationBody.physicalAddress.locationName || "N/A"}
                            </Text>
                            <Text>
                            ...
                            <Text>{civicData.state[0].electionAdministrationBody.absenteeVotingInfoUrl || "N/A"}</Text>
                            <Text>{civicData.state[0].electionAdministrationBody.votingLocationFinderUrl || "N/A"}</Text>
